...
const Tours = ({ slug }) => {
  ...
  const fetchTour = async () => {
    try {
      ...
      const res = await fetch(`/api/get-by-id-toures-section/${slug}`);
      const json = await res.json();

      setTour(json?.result?.data || null);
    } catch {
      setError("Tour load karne mein dikkat hui.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const CHILD_PRICE = Number(tour?.child || 0);
  ...
  const inpSx = {
    ...
    "& .MuiInputBase-input": { padding: "10px 13px" },
  ...
  // Image grid: 1 image = full width, 2 = half/half, 3+ = masonry-style
  const renderImages = () => {
    ...
    if (imgs.length === 1) {
      return (
        <Box sx={{ borderRadius: "12px", overflow: "hidden", height: { xs: 200, sm: 280 } }}>
          <Box component="img" src={imgs[0]} alt="tour-0"
            sx={{ width: "100%", height: "100%", objectFit: "cover", transition: "transform .35s", "&:hover": { transform: "scale(1.04)" } }} />
        </Box>
      );
    }
        ...
        <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
          {imgs.map((img, i) => (
            <Box key={i} sx={{ borderRadius: "12px", overflow: "hidden", aspectRatio: "4/3" }}>
              <Box component="img" src={img} alt={`tour-${i}`}
                sx={{ width: "100%", height: "100%", objectFit: "cover", transition: "transform .35s", "&:hover": { transform: "scale(1.04)" } }} />
            </Box>
          ))}
        </Box>
        ...
        <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gridTemplateRows: "auto auto", gap: "10px" }}>
          ...
          {imgs.slice(1).map((img, i) => (
            <Box key={i} sx={{ borderRadius: "12px", overflow: "hidden", aspectRatio: "16/9" }}>
              <Box component="img" src={img} alt={`tour-${i + 1}`}
                sx={{ width: "100%", height: "100%", objectFit: "cover", transition: "transform .35s", "&:hover": { transform: "scale(1.04)" } }} />
            </Box>
          ))}
        </Box>
        ...
        <Box sx={{ display: "grid", gridTemplateColumns: `repeat(${Math.min(imgs.length - 2, 3)}, 1fr)`, gap: "10px" }}>
          {imgs.slice(2, 5).map((img, i) => (
            <Box key={i} sx={{ borderRadius: "12px", overflow: "hidden", aspectRatio: "1/1", position: "relative" }}>
              <Box component="img" src={img} alt={`tour-${i + 2}`}
                sx={{ width: "100%", height: "100%", objectFit: "cover", transition: "transform .35s", "&:hover": { transform: "scale(1.04)" } }} />
              {i === 2 && imgs.length > 5 && (
  ...
  return (
    <>
      <Navbar />
      <Box sx={{ minHeight: "100vh", background: "#F5F4F0", fontFamily: "'Inter', -apple-system, sans-serif", py: { xs: 2.5, sm: 4 }, px: { xs: 1.5, sm: 2 }, pt: { xs: 10, sm: 12 }, }}>
        <style>{`@keyframes shimmer{0%{background-position:200% 0}100%{background-position:-200% 0}}`}</style>
        <Box sx={{ maxWidth: 1160, mx: "auto" }}>
              ...
              <Stack spacing={2}>
                ...
                <Box>
                  {tour.badge && (
                    <Box sx={{
                      ...
                      background: "#FFF3E0", color: "#854F0B", fontSize: 11, fontWeight: 700,
                      padding: "5px 13px", borderRadius: "20px", border: "1px solid #EF9F27", mb: "12px",
                    }}>
                      <LocalOffer sx={{ fontSize: 13 }} /> {tour.badge}
                    </Box>
                  )}
                  <Typography sx={{ fontSize: { xs: 24, sm: 30, md: 34 }, fontWeight: 800, color: "#18181B", lineHeight: 1.2, mb: "14px", letterSpacing: "-0.5px" }}>
                    {tour.title}
                  </Typography>
                  <Box sx={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                    <Box sx={{
                      display: "inline-flex", alignItems: "center", gap: "6px", fontSize: 13, fontWeight: 600,
                      padding: "6px 14px", borderRadius: "20px", border: "1px solid #F0D893",
                    ...
                    <Box sx={{
                      display: "inline-flex", alignItems: "center", gap: "6px", fontSize: 13, fontWeight: 600,
                      padding: "6px 14px", borderRadius: "20px", border: "1px solid #A5D6A7",
                ...
                <Card sx={cardSx}>
                  <CardContent sx={{ p: "18px 20px !important" }}>
                    ...
                    <Typography sx={{ fontSize: 14, color: "#52525B", lineHeight: 1.85 }}>
                      {tour.description}
                    </Typography>
                  </CardContent>
                </Card>
                  ...
                  {submitted ? (
                    ...
                    <Box sx={{ textAlign: "center", py: 1 }}>
                      ...
                      <Typography sx={{ fontSize: 13, color: "#71717A", lineHeight: 1.7, mb: "20px" }}>
                        {form.name}, aapki booking confirm hai.<br />
                      ...
                      <Button
                        fullWidth
                        onClick={() => { setSubmitted(false); setForm({ name: "", email: "", phone: "", guests: 1, children: 0, date: "", specialRequest: "" }); }}
                    ...
                    <Box component="form" onSubmit={handleSubmit}>
                      ...
                      <Stack spacing={1.2}>
                        ...
                        {/* Adults + Date */}
                        <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
                          <TextField select label="Adults" name="guests" value={form.guests}
                          ...
                          <TextField
                            ...
                            required
                            label="Tour Date"
                            // InputLabelProps ki jagah slotProps use karein
                            slotProps={{
                              inputLabel: {
                                shrink: true,
                              },
                        ...
                        <TextField
                          ...
                          fullWidth
                          sx={inpSx}
                          // InputProps ki jagah slotProps.input use karein
                          slotProps={{
                            input: {
                              startAdornment: (
                                <ChildCare sx={{ fontSize: 18, color: "#9B9590", mr: 1 }} />
                              ),
                            },
                          }}
                        >
                        ...
                        <TextField multiline rows={2} label="Special Request (optional)" name="specialRequest"
                          value={form.specialRequest} onChange={handleChange} fullWidth
                          sx={{ ...inpSx, "& .MuiInputBase-input": { padding: "10px 13px", resize: "none" } }} />
                      </Stack>
                      ...
                      <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "8px", mb: "14px" }}>
                        {[
                          ...
                          <Box
                            ...
                            sx={{
                              p: "9px 4px", fontSize: 11, fontWeight: 700,
                      ...
                      {paymentMethod === "card" && (
                        <Stack spacing={1.2} mb={1.5}>
                          ...
                          <TextField
                            ...
                            slotProps={{
                              htmlInput: { maxLength: 19 } // inputProps yahan shift ho gaya
                            }}
                            fullWidth
                            required
                            sx={inpSx}
                          ...
                          {/* Name on Card - Isme koi extra props nahi the toh ye waisa hi hai */}
                          <TextField
                            label="Name on Card"
                            value={card.name}
                          ...
                          <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
                            ...
                            <TextField
                              ...
                              slotProps={{
                                htmlInput: { maxLength: 5 } // yahan bhi slotProps use kiya
                              }}
                              fullWidth
                              required
                              sx={inpSx}
                      ...
                      {paymentMethod === "upi" && (
                        <Box mb={1.5}>
                          <TextField label="UPI ID (e.g. rahul@okaxis)" value={upi}
                            onChange={(e) => setUpi(e.target.value)} fullWidth required sx={inpSx} />
                      ...
                      {paymentMethod === "netbanking" && (
                        <TextField select label="Select Your Bank" value={bank}
                          onChange={(e) => setBank(e.target.value)} fullWidth required sx={{ ...inpSx, mb: 1.5 }}>
                      ...
                      <Button
                        ...
                        sx={{
                          ...
                          transition: "all .2s",
                          "&:hover": { background: "linear-gradient(135deg, #1D9E75 0%, #0F7A5A 100%)", opacity: submitting ? 0.7 : 0.9, boxShadow: "0 6px 18px rgba(29,158,117,0.4)" },
